...
def p_diode(x):
    if x <= 500:
        y = 6.6763288698E-12*x**5 - 1.3543608284E-08*x**4 + 1.0888317706E-05 * \
            x**3 - 4.3335323943E-03*x**2 + 8.5399725862E-01*x - 6.6558844853E+01
    elif 500 <= x < 950:
        y = 5.5585093168E-04*x - 2.0552365026E-02
    elif 950 <= x <= 1100:
        y = 3.1698368227E-09*x**4 - 1.2731281453E-05*x**3 + \
            1.9132245041E-02*x**2 - 1.2752168975E+01*x + 3.1818412914E+03
    return y


def read_file(uploaded_file) -> pd.DataFrame:
    try:
        df = pd.read_table(uploaded_file, encoding='cp1251',
                           skiprows=range(25), sep="  ")
...
with st.expander("Загрузка файлов"):
    ...
    uploaded_files = st.file_uploader(
        "Файлы данных", type=['edf', 'csv'], accept_multiple_files=True)
    ...
    if (data_valid):
        ...
        print(lin_sam)
        fig = px.line(currents_sample, x=max_key + "_Potential",
                      y=lin_sam, labels={'value': "Current"})
